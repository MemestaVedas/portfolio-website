...
// Helper for distinct letter animation
const ParallaxLetter = ({ char, progress, range }: { char: string, progress: MotionValue<number>, range: [number, number] }) => {
    const y = useTransform(progress, range, ['100%', '0%']);
    const opacity = useTransform(progress, range, [0, 0.15]); // Slightly higher opacity

    return (
        <motion.span
            style={{ y, opacity, fontFamily: 'var(--font-druk)' }}
            className="text-[14vw] md:text-[15vw] font-black text-white-pure leading-none"
        >
            {char}
        </motion.span>
    );
...
const Footer = () => {
    // Animation hooks for the "MEMESTA" wave
    const { scrollYProgress } = useScroll();

    // We map each letter's animation to a specific range of the total scroll
    // The footer is revealed in the last 15-20% of the page
    const letters = "MEMESTA".split("");

    return (
        <footer className="fixed bottom-0 left-0 right-0 h-screen md:h-[80vh] bg-electric-primary pt-24 pb-8 overflow-hidden z-0 flex flex-col justify-between" id="contact">
            ...
            <div className="absolute inset-0 pointer-events-none">
                ...
                {/* MEMESTA Wave Text - Edge to Edge with Scroll Parallax */}
                <div className="absolute inset-x-0 bottom-0 flex items-end justify-between px-2 pb-[2vh] pointer-events-none select-none overflow-hidden w-full">
                    {letters.map((char, i) => {
                        // Stagger the start time for each letter based on scroll progress
                        // The wave starts at 0.85 (scroll progress) and ends at 1.0
                        const start = 0.88 + (i * 0.015);
                        const end = 0.98 + (i * 0.005);
            ...
            <div className="container mx-auto px-8 relative z-10 max-w-6xl flex-grow flex flex-col justify-center">
                {/* CTA Section */}
            ...
            {/* Bottom bar */}
            <div className="relative z-10 container mx-auto px-8 border-t border-white-soft/20 pt-6 pb-6 flex flex-col md:flex-row justify-between items-center gap-4">
